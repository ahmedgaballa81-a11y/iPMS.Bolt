    ...
    @if (!authService.isAuthenticated()) {
          ...
          <nav class="nav">
            ...
            <div class="dropdown" [class.active]="dropdownState.settings">
              ...
              <div class="dropdown-menu">
                <button class="dropdown-item" (click)="navigateToCompanies(); closeDropdowns()">
                  🏢 Companies
                </button>
                <button class="dropdown-item" (click)="navigateToCustomers(); closeDropdowns()">
                  👥 Customers
                </button>
                <button class="dropdown-item" (click)="navigateToLocations(); closeDropdowns()">
                  📍 Locations
                </button>
                @if (authService.isAdmin()) {
                  <button class="dropdown-item" (click)="navigateToUsers(); closeDropdowns()">
                    👤 Users
                  </button>
                }
                <button class="dropdown-item" (click)="navigateToAuthorities(); closeDropdowns()">
                  🛡️ Authorities
                </button>
                <button class="dropdown-item" (click)="navigateToActivityLog(); closeDropdowns()">
                  📊 Activity Log
                </button>
                <button class="dropdown-item" (click)="navigateToCompanyActivityLog(); closeDropdowns()">
                  💼 Company Activities
                </button>
                <button class="dropdown-item" (click)="navigateToIndustries(); closeDropdowns()">
                  📦 Industries
                </button>
                <button class="dropdown-item" (click)="navigateToIndustries(); closeDropdowns()">
                  📝 Activities
            ...
            <div class="dropdown" [class.active]="dropdownState.notifications">
              <button 
                ...
                @if (getUserTasks().length > 0) {
                  <span class="notification-badge">{{ getUserTasks().length }}</span>
                }
              </button>
              <div class="dropdown-menu notifications-menu">
                ...
                @if (getUserTasks().length === 0) {
                  ...
                  @for (task of getUserTasks(); track task.id) {
                    <div class="dropdown-item task-item" (click)="navigateToTask(task); closeDropdowns()">
                      <div class="task-title">{{ task.title }}</div>
                      <div class="task-project">{{ getProjectTitle(task.projectId) }}</div>
                      <div class="task-status" [class]="'status-' + task.status">{{ task.status }}</div>
                    </div>
                  }
                }
              </div>
            ...
            <div class="dropdown" [class.active]="dropdownState.profile">
              <button 
                ...
                <div class="user-avatar">{{ (authService.user()?.name ?? '').charAt(0).toUpperCase() }}</div>
              </button>
              <div class="dropdown-menu profile-menu">
                <div class="dropdown-header">
                  <div class="user-name">{{ authService.user()?.name }}</div>
                  <div class="user-role" [class]="'role-' + authService.user()?.role">
                ...
                <button class="dropdown-item" (click)="viewProfile(); closeDropdowns()">
                  👤 My Profile
                </button>
                <button class="dropdown-item" (click)="changePassword(); closeDropdowns()">
                  ⚙️ Change Password
                </button>
                <div class="dropdown-divider"></div>
                <button class="dropdown-item logout-item" (click)="logout(); closeDropdowns()">
                  🚪 Sign Out
                </button>
              </div>
            </div>
      ...
      <main class="main">
        @switch (currentView()) {
          ...
          @case ('activity-log') {
            <app-user-management [initialView]="'activity'" />
          }
          @case ('company-activity-log') {
            <app-company-activity-log />
          }
          @case ('industries') {
            <app-industry-list />
          }
...
export class AppComponent {
  ...
  readonly currentView = computed(() => this.navigationService.currentState().currentView);
  ...
  navigateToIndustries(): void { // New navigation method
  ...
  isSettingsActive(): boolean {
    ...
    return view === 'locations' || view === 'users' || view === 'authorities' || view === 'activity-log' || view === 'company-activity-log' || view === 'industries' || view === 'activities';
  }

  getUserTasks() {
    // Get tasks assigned to current user (simplified - in real app would filter by assignee)
    return this.dataService.allTasks().filter(task => 
  ...
  navigateToTask(task: any): void {
    ...
    if (project) {
      this.navigationService.navigateToTasks(project.customerId, project.companyId, task.projectId);
    }
  }
